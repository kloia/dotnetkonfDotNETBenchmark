...
WORKDIR /src
COPY ["NET5ContainerImage.csproj", "NET5ContainerImage/"]
RUN dotnet restore "NET5ContainerImage/NET5ContainerImage.csproj"

WORKDIR "/src/NET5ContainerImage"
COPY . .
RUN dotnet build "NET5ContainerImage.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "NET5ContainerImage.csproj" \
...
Execute unit tests
```
cd "NET5ContainerImage/test/NET5ContainerImage.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "NET5ContainerImage/src/NET5ContainerImage"
